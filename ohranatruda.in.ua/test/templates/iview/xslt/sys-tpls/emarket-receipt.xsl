	...
	<xsl:template match="/udata/object" mode="print-reciept">
		...
		<html>
			<head id="header">
				<script>
					<![CDATA[
					var pathArray = location.pathname.trim('/').split('/');
					var slicePosition = pathArray.lastIndexOf('emarket') + 1;
					var prefix = pathArray.slice(0, slicePosition).join('/') + '/';
					
					window.onload = function() {
 						if (window.opener) window.opener.document.location.href = prefix + 'ordersList/?' + Math.random();
					}
					]]>
				</script>
